...
1. **Ingestão** (`app/ingestion.py`): lê as fontes `active=True` da tabela `source`. Para fontes `RSS`, extrai os links via `feedparser`. Aplica o filtro anti-ruído de `NEGATIVE_KEYWORDS` (título/URL) antes de gastar uma extração, deduplica contra URLs já salvas e respeita `max_daily_articles` por fonte. Para cada link novo aprovado, baixa a página e extrai o texto principal com `trafilatura.fetch_url` + `trafilatura.extract`, salvando o artigo como `PENDING`. Os artigos de uma mesma fonte são processados **estritamente em sequência** (um por vez, com uma pausa de 2s entre eles) para manter o uso de CPU/RAM baixo.
